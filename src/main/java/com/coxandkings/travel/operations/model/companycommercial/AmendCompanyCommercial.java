...
@Table(name="Company_Commercial")
@Entity
@TypeDefs( {@TypeDef( name= "StringJsonObject", typeClass = JSONUserType.class)})
public class AmendCompanyCommercial extends BaseModel {
    public AmendCompanyCommercial() {
	}
	/**
	 * 
	 */
	...
	@JoinTable(name="Company_Commercial_Selling_PRICE_COMPONENT",joinColumns =
            @JoinColumn(name = "company_commercial_id", referencedColumnName = "id"),
            inverseJoinColumns = @JoinColumn(name = "selling_price_component"))
    private Set<SellingPriceComponent> sellingPriceComponent;
	
	@ManyToMany(fetch = FetchType.LAZY,cascade=CascadeType.ALL)
	@JoinTable(name="Company_Commercial_APPLY_ON",joinColumns =
            @JoinColumn(name = "company_commercial_id", referencedColumnName = "id"),
            inverseJoinColumns = @JoinColumn(name = "apply_commercial_on"))
    private Set<ApplyCommercialOn> applyOnProducts;
	
	@ManyToMany(fetch = FetchType.LAZY,cascade=CascadeType.ALL)
	@JoinTable(name="Company_Commercial_Ineligible_Commercial",joinColumns =
            @JoinColumn(name = "company_commercial_id", referencedColumnName = "id"),
